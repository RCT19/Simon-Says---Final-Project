...
//progresses this variable is decremented which decreases the amount of time the user has to remeber the sequence, thus
...
//level, which increases the difficulty of the game by requiring the user to remeber longer sequences as the game
//progresses.
...
//this part of the program is resonsible for lighting up a sequence of LEDs that indicates to the user that the game is 
...
//this for loop is resonsible for controlling the amount of LEDs turned on per round. The amount of LEDs that turn on 
...
//this if statement is resonsible for turning on the blue LED and playing a 2000 Hertz sound for a second if the random 
//number is a one. After the one second the program then shuts off the buzzer and LED.
...
//this if statement is resonsible for turning on the green LED and playing a 3000 Hertz sound for a second if the
...
//button is not pressed then the LED and buzzer will remain off. In addition, the inner if statement is resonsible for
//breaking the sequence is the user inputs the wrong sequence. Once the program detects that the user inputed the
...
//the button is not pressed then the LED and buzzer will remain off. In addition, the inner if statement is resonsible 
//for breaking the sequence is the user inputs the wrong sequence. Once the program detects that the user inputed the 
//incorrect sequence, the if statement breaks and executes the failure statements.     
...
//this if-else statement tests to see if the user inputed the correct sequence. If the user inputed the correct
//sequence then the value of failure remains false and the program increments the value of level, reduces the amount of
//time the user has to remeber the sequence and also displays a light pattern indicating to the user that they entered 
//the correct sequence and the next level is going to begin. If the user enters the sequence incorrectly, then the 
//value of failure becomes true and the program displays a light pattern indicating to the user that they inputed the 
...
//this if statement is resonpsible for testing if the value of lives is equal to zero. If the value of lives is equal
